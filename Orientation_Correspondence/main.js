...
/* Color List */
var colors = ['black', 'black', 'black', 'black', 'black', 'black', 'black', 'black']
...
function drawStimuli(trial, stimuliListLeft, stimuliListRight) {
    ...
    orientList = generateOrientation(trial[0], orientList)
    for (let i = 0; i < trial[0]; i++) {
        stimuliListLeft[i].className = "square";
        stimuliListLeft[i].style = 'transform: rotate(' + orientList[i] + 'deg)';
        ...
        stimuliListRight[i].style = 'transform: rotate(' + orientList[i] + 'deg)';
    ...
    orientList_left = orientList;
    orientList_right = orientList;
    if (trial[1] == 'yes') {
        randomNum = Math.floor(getRandomArbitrary(0, trial[0]));
        let tempLeft = stimuliListRight[randomNum].style.left;
        let tempTop = stimuliListRight[randomNum].style.top;
        let tempBack = stimuliListRight[randomNum].style.backgroundColor;
        orientList_right[randomNum] = orientList_right[randomNum] + 90;
        stimuliListRight[randomNum].style = 'transform: rotate(' + orientList_right[randomNum] + 'deg)';
        stimuliListRight[randomNum].style.left = tempLeft;
        stimuliListRight[randomNum].style.top = tempTop;
        stimuliListRight[randomNum].style.backgroundColor = tempBack; 
    }
    start = Date.now();
}
...
function clearTrial(keyPress, ans, rt) {
    ...
    for (let i = 0; i < trial_list[curr][0]; i++) {
        left.push(orientList_left[i]);
        right.push(orientList_right[i]);
    }
    if (trial_list[curr][1] == 'yes') {
        right[randomNum] = right[randomNum] + 90;
    }
    let left_pos_color = {};
    let right_pos_color = {};
    for (let i = 0; i < trial_list[curr][0]; i++) {
        ...
        if (i == trial_list[curr][0] - 1) {
            ...
            'reaction_time': rt, 'resp': keyPress, 'correct': correct, 'left_pos_color': left_pos_color, 'right_pos_color': right_pos_color, 'orient_left': left, 'orient_right': right});
...
function randomNumLeftX() {
    return getRandomArbitrary(119, 566);
}

/* Generates random Y position within the correct pixel boundry. */
function randomNumLeftY() {
    return getRandomArbitrary(76, 524); 
}

function randomOrientLeft() {
    return getRandomArbitrary(-180, 180);
}
...
function generateOrientation(load, orientList) {
    for (let i = 0; i < load; i++) {
        orientList[i] = randomOrientLeft();
    }
    return orientList;
}
