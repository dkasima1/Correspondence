...
var copy_colors = ['red', 'blue', 'green', 'orange', 'yellow', 'pink', 'gray', 'purple'];
...
var stimuliListLeft = [];
var stimuliListInsideLeft = [];

/* Stimuli List for Right Side */
var stimuliListRight = [];
var stimuliListInsideRight = [];
...
function start_trial(curr, numTrials, trial_list) {
    if (curr < numTrials) {
        drawStimuli(trial_list[curr], stimuliListLeft, stimuliListRight, stimuliListInsideLeft, stimuliListInsideRight);
...
//create random string for participant ID
 ...
 */
function drawStimuli(trial, stimuliListLeft, stimuliListRight, stimuliListInsideLeft, stimuliListInsideRight) {
    shuffle(colors);
    for (let i = 0; i < trial[0]; i++) {
        stimuliListLeft.push(document.createElement('div'));
        stimuliListRight.push(document.createElement('div'));
        stimuliListInsideLeft.push(document.createElement('div'));
        stimuliListInsideRight.push(document.createElement('div'));
    }
    ...
    while (sameColors(colors, copy_colors)) {
        shuffle(copy_colors);
    }
    
    for (let i = 0; i < trial[0]; i++) {
        //InsideLeft
        stimuliListInsideLeft[i].className = "inside_square";
        stimuliListInsideLeft[i].style.left = positionList[i][0] + 12.5 +'px';
        stimuliListInsideLeft[i].style.top = positionList[i][1]+ 12.5 + 'px';
        stimuliListInsideLeft[i].style.backgroundColor = copy_colors[i];
        document.getElementsByTagName("body")[0].appendChild(stimuliListInsideLeft[i]);
        
        //InsideRight
        stimuliListInsideRight[i].className = "inside_square";
        stimuliListInsideRight[i].style.left = positionList[i][0] + 587 + 12.5 + 'px';
        stimuliListInsideRight[i].style.top = positionList[i][1] + 12.5 + 'px';
        stimuliListInsideRight[i].style.backgroundColor = stimuliListInsideLeft[i].style.backgroundColor;
        document.getElementsByTagName("body")[0].appendChild(stimuliListInsideRight[i]);
    }
    if (trial[1] == 'yes') {
        var randomNum = Math.floor(getRandomArbitrary(0, trial[0]));
        let anoRandom = Math.floor(Math.random() * 10);
        if (anoRandom > 4) {
            if (copy_colors[randomNum] == colors[7]) {
                if (randomNum == 0) {
                    randomNum++;
                } else {
                    randomNum--;
            ...
            if (colors[randomNum] == copy_colors[7]) {
                if (randomNum == 0) {
                    randomNum++;
                } else {
                    randomNum--;
            ...
            stimuliListInsideRight[randomNum].style.backgroundColor = copy_colors[7];
        }
    }
    start = Date.now();
}
...
function clearTrial(keyPress, ans, rt) {
    ...
    for (let i = 0; i < trial_list[curr][0]; i++) {
        left_pos_color[[stimuliListLeft[i].style.backgroundColor, stimuliListInsideLeft[i].style.backgroundColor]] = [[stimuliListLeft[i].style.left, stimuliListLeft[i].style.top], [stimuliListInsideLeft[i].style.left, stimuliListInsideLeft[i].style.top]];
        right_pos_color[[stimuliListRight[i].style.backgroundColor, stimuliListInsideRight[i].style.backgroundColor]] = [[stimuliListRight[i].style.left, stimuliListRight[i].style.top], [stimuliListInsideRight[i].style.left, stimuliListInsideRight[i].style.top]];
        bod.removeChild(stimuliListRight[i]);
        bod.removeChild(stimuliListLeft[i]);
        bod.removeChild(stimuliListInsideRight[i]);
        bod.removeChild(stimuliListInsideLeft[i]);
...
function sameColors(colors, copy_colors) {
    for (let i = 0; i < colors.length; i++) {
        if (colors[i] == copy_colors[i]) {
            return true;
        }
    }
    return false;
